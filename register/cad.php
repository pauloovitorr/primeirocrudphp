...
if($_SERVER['REQUEST_METHOD']=== 'POST' && !empty($_POST['nome']) && !empty($_POST['email']) && !empty('senha') )
{
  $nome = $conecte->real_escape_string($_POST['nome']);
  $email = $conecte->real_escape_string($_POST['email']);
  $senha = $conecte->real_escape_string($_POST['senha']);
  ...
  if($dados->errno)
  {
    echo 'Não foi possível cadastrar usuário'. $dados->errno;
  }
  else
  {
